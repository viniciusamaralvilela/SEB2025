...
static const uint8_t
  init_cmds1[] = {            // Init for 7735R, part 1 (red or green tab)
    ...
    ST7735_PWCTR4 , 2      ,  // 10: Power control, 2 args, no HAL_Delay:
      ...
      0x2A,  
    ST7735_PWCTR5 , 2      ,  // 11: Power control, 2 args, no HAL_Delay:
      0x8A, 0xEE,
    ST7735_VMCTR1 , 1      ,  // 12: Power control, 1 arg, no HAL_Delay:
      0x0E,
    ST7735_INVOFF , 0      ,  // 13: Don't invert display, no args, no HAL_Delay
  ...
  init_cmds3[] = {            // Init for 7735R, part 3 (red or green tab)
    ...
    ST7735_GMCTRP1, 16      , //  1: Gamma Adjustments (pos. polarity), 16 args, no HAL_Delay:
      0x02, 0x1c, 0x07, 0x12,
      0x37, 0x32, 0x29, 0x2d,
      0x29, 0x25, 0x2B, 0x39,
      0x00, 0x01, 0x03, 0x10,
    ST7735_GMCTRN1, 16      , //  2: Gamma Adjustments (neg. polarity), 16 args, no HAL_Delay:
      0x03, 0x1d, 0x07, 0x06,
      0x2E, 0x2C, 0x29, 0x2D,
      0x2E, 0x2E, 0x37, 0x3F,
      0x00, 0x00, 0x02, 0x10,
...
void ST7735_DrawLines()
{
	ST7735_FillScreen(BLACK);
	  for (int16_t x=0; x < ST7735_WIDTH; x+=6) {
		ST7735_DrawLine(0, 0, x, ST7735_HEIGHT-1, YELLOW);
	    HAL_Delay(0);
	  }
	  for (int16_t y=0; y < ST7735_HEIGHT; y+=6) {
	    ST7735_DrawLine(0, 0, ST7735_WIDTH-1, y, YELLOW);
	    HAL_Delay(0);
	  }

	  ST7735_FillScreen(BLACK);
	  for (int16_t x=0; x < ST7735_WIDTH; x+=6) {
	    ST7735_DrawLine(ST7735_WIDTH-1, 0, x, ST7735_HEIGHT-1, YELLOW);
	    HAL_Delay(0);
	  }
	  for (int16_t y=0; y < ST7735_HEIGHT; y+=6) {
	    ST7735_DrawLine(ST7735_WIDTH-1, 0, 0, y, YELLOW);
	    HAL_Delay(0);
	  }

	  ST7735_FillScreen(BLACK);
	  for (int16_t x=0; x < ST7735_WIDTH; x+=6) {
	    ST7735_DrawLine(0, ST7735_HEIGHT-1, x, 0, YELLOW);
	    HAL_Delay(0);
	  }
	  for (int16_t y=0; y < ST7735_HEIGHT; y+=6) {
	    ST7735_DrawLine(0, ST7735_HEIGHT-1, ST7735_WIDTH-1, y, YELLOW);
	    HAL_Delay(0);
	  }

	  ST7735_FillScreen(BLACK);
	  for (int16_t x=0; x < ST7735_WIDTH; x+=6) {
	    ST7735_DrawLine(ST7735_WIDTH-1, ST7735_HEIGHT-1, x, 0, YELLOW);
	    HAL_Delay(0);
	  }
	  for (int16_t y=0; y < ST7735_HEIGHT; y+=6) {
	    ST7735_DrawLine(ST7735_WIDTH-1, ST7735_HEIGHT-1, 0, y, YELLOW);
	    HAL_Delay(0);
	  }
}
